...
class SinglePostScreen1 extends StatefulWidget {
  ...
  const SinglePostScreen1(this.id,
...
class _SinglePostScreen1State extends State<SinglePostScreen1> {
  ...
  void initState() {
    ...
    super.initState();
    Provider.of<NewsFeedProvider>(context, listen: false).callForSinglePosts(widget.id);
    Provider.of<CommentProvider>(context, listen: false).initializeCommentData(widget.commentUrl);
    if (widget.isFromGroup || widget.isFromPage) {
      Provider.of<CommentProvider>(context, listen: false).initializeSocket(int.parse(widget.id));
    } else {
      Provider.of<CommentProvider>(context, listen: false).initializeSinglePostSocket(widget.commentUrl);
    }
    ...
    controller.addListener(() {
      if (controller.offset >= controller.position.maxScrollExtent &&
        ...
        Provider.of<CommentProvider>(context, listen: false).updatePageNo(widget.commentUrl);
      }
    });
  }
                  ...
                  child: Column(
                    children: [
                      commentProvider.isShowCancelButton
                          ...
                          : commentProvider.isPressUpdateButton
                              ? Row(
                                  children: [
                                    const SizedBox(width: 15),
                      ...
                      TextField(
                        ...
                        decoration: InputDecoration(
                            suffixIcon: commentProvider.isCommentLoading
                                ...
                                : IconButton(
                                    onPressed: () {
                                      if (commentProvider.isPressUpdateButton) {
                                        FocusScope.of(context).unfocus();
                                        commentProvider.updateComment(widget.commentUrl, commentController.text).then((value) {
                                          if (value) {
                                            commentController.clear();
                                            commentProvider.resetUpdateButton();
                                          }
                                        });
                                      } else if (commentProvider.isShowCancelButton) {
                                        FocusScope.of(context).unfocus();
                                        commentProvider.addReply(commentController.text, widget.commentUrl).then((value) {
                                          if (value) {
                                            commentController.clear();
                                        ...
                                        commentProvider
                                            .addComment(
                                                ...
                                                newsFeedProvider.singleNewsFeedModel.id! as int,
