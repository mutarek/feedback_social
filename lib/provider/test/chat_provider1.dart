...
class ChatProvider1 with ChangeNotifier {
  final ChatRepo1 chatRepo1;
  final AuthRepo1 authRepo1;

  ChatProvider1({required this.chatRepo1, required this.authRepo1});
...
//TODO; Get ALl Chats
  ...
  initializeAllChats({int page = 1, bool isFirstTime = true}) async {
    ...
    // notifyListeners();
    ApiResponse apiResponse = await chatRepo1.getUserAllChatLists(page);
    _isLoading = false;
    isBottomLoading = false;
...
//TODO; Get p2p Chats
  ...
  initializeP2PChats(Function callBack,
    ...
    if (isFromChatTwoUser) {
      apiResponse =
      await chatRepo1.getChatMessageBetweenTwoUser(userID.toString(), page);
    } else {
      apiResponse = await chatRepo1.getUserP2PChatLists(chatModels.id!, page);
    }
  ...
  String userName() {
    return authRepo1.getUserName();
  }

  String userID() {
    return authRepo1.getUserID();
  }
  ...
  Future<int> createRoom(
    ...
    _isLoading = true;
    notifyListeners();
    ApiResponse apiResponse = await chatRepo1.callForGetRoomID(customerID);
    _isLoading = false;
    isOneTime = false;
    if (apiResponse.response.statusCode == 200) {
