...
class CommentProvider1 with ChangeNotifier {
  final CommentRepo1 commentRepo1;

  CommentProvider1({required this.commentRepo1});
  ...
  void initializeCommentData(String url, {int page = 1}) async {
    ...
    ApiResponse response = await commentRepo1.getAllCommentData(url);
    isLoading = false;
    isBottomLoading = false;
  ...
  void initializeGroupCommentData(int postID, int groupID) async {
    ...
    comments = [];
    isLoading = true;
    ApiResponse response = await commentRepo1.getAllGroupCommentData(postID, groupID);
    isLoading = false;
    if (response.response.statusCode == 200) {
  ...
  Future<bool> addComment(String comment, String fullName, String profileImage, int postID, int userID, String url) async {
    ...
    ApiResponse response = await commentRepo1.addComment(url, comment);
  ...
  Future<bool> addReply(String comment, String url) async {
    isCommentLoading = true;
    notifyListeners();
    ApiResponse response = await commentRepo1.addReply(url, comments[replyIndex].id.toString(), comment);
    isCommentLoading = false;
    if (response.response.statusCode == 201) {
      Replies c = Replies(
